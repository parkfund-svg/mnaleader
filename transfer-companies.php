...
// 전체 개수
$countStmt = $pdo->prepare("SELECT COUNT(*) FROM transfer_companies WHERE status = 'available' AND $whereClause");
...
// 데이터 조회 (거래가능만)
$sql = "SELECT * FROM transfer_companies WHERE status = 'available' AND $whereClause ORDER BY created_at DESC LIMIT $perPage OFFSET $offset";
        ...
        <form method="GET" action="">
            ...
            <div class="category-section">
                <div class="filter-actions">
                    ...
                    <a href="<?php echo SITE_URL; ?>/transfer-companies.php" class="btn-reset">초기화</a>
                </div>
            </div>
        </form>
                ...
                <table class="results-table">
                    ...
                    <tbody>
                        <?php foreach ($companies as $company): ?>
                            <tr onclick="window.location.href='<?php echo SITE_URL; ?>/transfer-company.php?id=<?php echo $company['id']; ?>'">
                                <td><span class="company-link"><?php echo sanitize($company['id']); ?></span></td>
                                <td><span class="status-badge">거래가능</span></td>
                                <td><?php echo sanitize($company['business_type']); ?></td>
                                <td><?php echo sanitize($company['construction_capability'] ?? '-'); ?></td>
                                <td><?php echo sanitize($company['cooperative_shares'] ?? '-'); ?></td>
                                <td><?php echo sanitize($company['company_type'] ?? '-'); ?></td>
